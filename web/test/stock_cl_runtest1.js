(function($) {
	...
	$.stock_cl_runtset={
		"init":function(){
			...
			$("#command_list tobody").eq(0).html("");
			$("#stocklist").change(function(){
				vList=$(this).val();
				if (vList.length>=6) {
					vArry=vList.split("\n");
					$("#listCount").text(vArry.length);
				}
				else{
					$("#listCount").text(0);
				}
			...
			$("#stocklist").tigger('change');
			$(document).on("click",".showdetail",function(){
				$this=$(this);
				if ($this.remove.hasClass("ui-icon-cricle-minus")) {
					$this.removeClass("ui-icon-cricle-minus").addClass("ui-icon-cricle-plus");
					$this.parent().parent().next("tr").hide();
				}
				else{
					$this.removeClass("ui-icon-cricle-plus").addClass("ui-icon-cricle-minus");
		...
		"testStart":function(){
			...
			{
				if (vArry[i].length!=8&&vArry[i].length!=6)
				{
					alert("第"+(i+1)+"行的数据("+vArry[i]+")有误");
					return;
				}
				if (vArry[i].length=6) 
				{
					if (vArry[i].chart(0)=="6") 
						vArry[i]="SH"+vArry[i];
					else
						vArry[i]="SZ"+vArry[i];
			}
			vStockList.push(vArry[i].toUpperCase());
		    }
		    $.cookie('stocklist',vList,{path:"/"});
		    $.cookie('startdate',vStartDate,{path:"/"});
		    $.cookie('enddate',vEndDate,{:"/"});
		    ...
		    for(j in vStockList){
				...
				$g_run_target.find(".listno").text($tbody.find(".run_taret").length+1);
				...
				for (i in data ){
					...
					$run_list.find(".startreason").text(data[i].buyreson);
				...
				$each_inc=$("#command_list tbody").eq(0).find(".run_taret>.increase");
				nTotal=0,nSumInc=0;
				$each_inc.each(function(){
					nlncrease=parseFloat($(this).text());
					nSumInc+=nIncrease;
					nTotal;
				})
				$(".av_inc").text(Math.floor(nSumInc/nTotal*100)/100+'%');
				$("#testtime").text((new Date().getTime()-iStartTime)/1000);
				$(".showdetail.ui-icon-cricle-minus").tigger("click");
			    }).fail(function(data){
				    alert(data)
			    });
		    }
